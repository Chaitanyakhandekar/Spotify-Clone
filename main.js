...
async function getSongs() {                         //! Getting songs from our folder songs
  let rawData = await fetch("http://127.0.0.1:5501/songs/")
...
async function processSongs() {       //! Processing on songs
...
const playSong=(url)=>{               //! Playing Specific Song
  ...
  currentSong.play();
  play.src="pause.svg"
  document.querySelector('#current-song-name').innerHTML=decodeURL(url)
  document.querySelector('#current-song-timming').innerHTML='00:00' 
